...
\newcommand{\educationalaids}{
	...
	\subsection*{During Invigilated Assessments}
	All aids on assessments are prohibited except those explicitly permitted as indicated on each assessment. In particular, devices capable of network connectivity or communication (sending and/or receiving) must be out of reach  during assessments. Students with access to these items are deemed to have an unauthorized aid during assessments.

	During quizzes, only a writing implements, and non-programmable calculator is allowed.
	
	During tests/exams, students will be permitted static formula/reference material/
	
	Exact details will be provided on moodle and announced in class each assessment approaches.
	...
	During assignments, lectures, and seminars, The course will primarily use R software, which is free to download and operate on Mac, Windows, and Linux. The institution provides computer Windows labs. 
	The permitted use of an LLM will be identified on each assignment. 

	
	\subsection*{Contacting Me}
	I'm available in person through the office hours listed above. If you are unable to attend the next office hour, you may schedule an appointment. I'm also available through the email above. Please note that I do not answer questions related to an assessment 24 hours prior to the deadline or assessment.  
...
\end{tabular}\hfill
\begin{tabular}{ r l}
	 \textbf{email:} & \href{mailto:\instructoremail}{\instructoremail}\\[1.5em]
	 %\textbf{Office Hours:} 	& \instructorphone
	~
\end{tabular}
...
Thompson Rivers University campuses are on the ancestral lands of the Tk'eml\'ups te Secw\'epemc (Kamloops campus) and the T'exelc (Williams Lake campus) within Secwepemc\'u$^{^{'}}$\hspace{-4pt}lecw, the traditional and unceded territory of the Secw\'epemc. The region TRU serves also extends into the territories of the St'\'at'imc, Nlaka'pamux, Nuxalk, T\v{s}ilhqot'in, Dakelh, and Syilx peoples.
